...
namespace WASender
{
    public partial class RunSingle : MaterialForm
    {
                ...
                foreach (var file in mesageModel.files)
                {
                    ...
                    if (ext == ".mp4")
                    {
                        try
                        {
                            ...
                            if (file.attachWithMainMessage == true)
                            {
                                if (generalSettingsModel.browserType == 1)
                                {
                                    WAPIHelper.SendVideo(driver, item.number, fullBase64, NewMessage, file.fileName);
                                }
                                else if (generalSettingsModel.browserType == 2)
                                {
                                    WPPHelper.SendVideoAsync(wv, item.number, fullBase64, NewMessage);
                                }
                    ...
                    else if (ext == ".ogg")
                    {
                        try
                        {
                            ...
                            //else if (generalSettingsModel.browserType == 2)
                            //{
                            //    WPPHelper.OpenChatopenChatBottomAsync(wv, item.number);
                            //    Thread.Sleep(500);
                            //    WPPHelper.markIsComposing(wv, item.number + "@c.us", rand);
                            //}
                            ...
                            if (file.attachWithMainMessage == true)
                            {
                                if (generalSettingsModel.browserType == 1)
                                {
                                    WAPIHelper.SendAudio(driver, item.number, fullBase64, NewMessage, mesageModel.buttons);
                                }
                                else if (generalSettingsModel.browserType == 2)
                                {
                                    WPPHelper.SendAudioAsync(wv, item.number, fullBase64, NewMessage);
                                }
                            ...
                            else
                            {
                                ...
                                {
                                    WPPHelper.SendAudioAsync(wv, item.number, fullBase64, file.Caption);
                                }
                            }
                        }
                        catch (Exception eeex)
                        ...
                        try
                        {
                            ...
                            if (file.attachWithMainMessage == true)
                            {
                                ...
                                else if (generalSettingsModel.browserType == 2)
                                {
                                    ...
                                    WPPHelper.SendVideoAsync(wv, item.number, fullBase64, NewMessage, file.fileName);
                                }
        ...
        //                        await wv.ExecuteScriptAsync("opeChatAtBottom('" + item.number + "@c.us')");
        //                        await Task.Delay(500);
